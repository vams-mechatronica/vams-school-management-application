...
def send_signout_reminder():
    ...
    missing_signouts = StaffAttendance.objects.filter(date=today, time_in__isnull=False, time_out__isnull=True)
    sitename = getattr(settings, 'SITE_NAME', 'School Portal')

    for record in missing_signouts:
        user = record.staff_user.user  # Assuming StaffAttendance has staff_user FK → Staff → user
        email = user.email
        if email:
            send_html_email_async(to_emails=[email],template_name="signout_remider",content_context={
